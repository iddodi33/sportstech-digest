...
{adapter_rows}  </tbody>
    ...
    auto_rej_ai      = [r for r in relevant if r.auto_rejected_reason == "ai_tech_ireland"]
    auto_rej_recur   = [r for r in relevant if r.auto_rejected_reason == "recurring_duplicate"]
...
<table style="border-collapse:collapse;">
  ...
  <tr><td style="padding:3px 20px 3px 0;color:#666;">&nbsp;&nbsp;auto-rejected (ai_tech_ireland)</td><td style="color:#666;">{len(auto_rej_ai)}</td></tr>
  <tr><td style="padding:3px 20px 3px 0;color:#666;">&nbsp;&nbsp;auto-rejected (recurring duplicate)</td><td style="color:#666;">{len(auto_rej_recur)}</td></tr>
  ...
  {cat_rows}
    ...
    # Excludes auto-rejected rows (ai_tech_ireland, recurring duplicates) —
    # they were inserted but immediately flipped to rejected, so they aren't
    # actually awaiting review.
    new_events = [r for r in relevant if r.was_inserted and not r.auto_rejected_reason]
    if new_events:
        event_rows = "".join(
            f"<tr>"
...
<p style="margin-top:8px;">
  <a href="https://hub.sportsd3c0d3d.ie/admin/events">Review at /admin/events &rarr;</a>
    ...
    # ── 7. Archive sweep ───────────────────────────────────────────────────────
    if sweep_result:
        sweep_failed = sweep_result.get("status") == "failed"
        sweep_color  = "#cc0000" if sweep_failed else "#2a7a2a"
        sweep_label  = sweep_result.get("error_message") or "OK"
        section7 = f"""\
<h3>Archive Sweep</h3>
<table style="border-collapse:collapse;">
  <tr><td style="padding:3px 20px 3px 0;">Stale pending events rejected (date passed)</td><td><strong>{sweep_result.get("rejected", 0)}</strong></td></tr>
  <tr><td style="padding:3px 20px 3px 0;">Status</td><td style="color:{sweep_color};">{_h(sweep_label if sweep_failed else "OK")}</td></tr>
</table>"""
    else:
        section7 = ""
    ...
    if silent:
        ...
        section8 = f"<h3>Sources Discovering Nothing This Run</h3><ul>{items}</ul>"
    else:
        section8 = ""
    ...
    body = (
        ...
        + (divider + section8 if section8 else "")
        + divider
